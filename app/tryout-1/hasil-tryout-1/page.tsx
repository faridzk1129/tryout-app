  ...
  return (
    <div className="mb-8">
      ...
      <div className="relative h-4 w-full bg-slate-100 rounded-full overflow-hidden border border-slate-200">
        <div
          className={`absolute top-0 left-0 h-full transition-all duration-1000 ease-out ${isPassed ? "bg-indigo-500" : "bg-orange-400"}`}
      ...
      <div className="flex justify-between mt-1 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
        <span>0</span>
        <span className="text-red-500">Ambang Batas: {pg}</span>
        <span>Max: {max}</span>
      </div>
    </div>
  );
          ...
          <div className="flex flex-col md:flex-row items-center gap-12 mb-16">
            ...
            <div className="flex-1 space-y-4">
              ...
              >
                ...
                <div>
                  <p className="text-slate-500 text-xs font-bold uppercase">Status Kelulusan</p>
                  <p
                    className={`text-xl font-black ${isLolosSemua ? "text-green-900" : "text-red-900"}`}
                  >
                    {isLolosSemua ? "LOLOS AMBANG BATAS 🎉" : "BELUM LOLOS AMBANG BATAS"}
                  </p>
                </div>
              </div>
            </div>
